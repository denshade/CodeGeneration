 ...
 */
public class MaxCostAccumulatorMatchAnyRegisterProgramFitnessExaminer implements ProgramFitnessExaminerInterface {
     ...
     */
    public MaxCostAccumulatorMatchAnyRegisterProgramFitnessExaminer(List<TestcaseInOutParameters> conditions, AccStatementRunner runner)
    {
     ...
     * @param conditions Conditions that define the input parameters & the expected outcome.
     */
    public MaxCostAccumulatorMatchAnyRegisterProgramFitnessExaminer(List<TestcaseInOutParameters> conditions, AccStatementRunner runner, String expectedResultRegister)
    {
    ...
    public double calculateFitness(List<InstructionMark> instructions, List<Register> registers)
    {
        ...
        double sum = 0.0;
        for (TestcaseInOutParameters p : conditions) {
            Double calculateError = AnyRegisterProgramTestcaseFitness.calculateError(runner, program, p, expectedResultRegister, noFitAtAll);
            double doubleValue = calculateError.doubleValue();
            sum += doubleValue;
            if (sum > maxCost) {
                break;
            }
